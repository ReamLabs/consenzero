...
use consensus::beacon_state::BeaconState;
...
async fn main() {
    ...
    let pre_state = BeaconState::deserialize(&pre_state_ssz).unwrap();

    let pre_state_ssz_rs = BeaconState::deserialize(&pre_state_ssz).unwrap();
    ...
    let block = BeaconBlock::deserialize(&block_ssz).unwrap();
    ...
    let env = ExecutorEnv::builder()
        .write(&pre_state.slot)
        .unwrap()
        .write(&pre_state.latest_block_header)
        .unwrap()
        .write(&pre_state.get_beacon_proposer_index().unwrap())
        .unwrap()
        .write(&block)
        .unwrap()
        .build()
